  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "HighWay의 단순함·HirePicker의 과한 설계가 각자 다음 프로젝트에서 결정 근거로 살아 움직이는 걸 직접 겪고 나서야, 이렇게 사이클을 만들어 가는 게 책 한 권보다 빠르다는 걸 느꼈다.",
            "HTTP에서 @AuthenticationPrincipal이 자연스럽게 동작하던 패턴을 WebSocket에 그대로 옮겼다가 null 디버깅을 한참 한 뒤에야, 같은 Spring이라도 통신 모델이 바뀌면 인증 컨텍스트가 어떻게 전달되는지부터 다시 봐야 한다는 걸 알게 됐다.",
            "웹에서 HttpOnly Cookie로 깔끔하게 풀리던 흐름이 Flutter에선 첫 요청부터 401로 막히는 걸 보고 나서야, 토큰 자체보다 어디 저장하고 어떻게 헤더에 싣느냐가 인증의 진짜 어려움이라는 걸 받아들였다.",
            "채팅 메시지를 MySQL 컬럼에 욱여넣다 첨부·이모지·시스템 메시지가 들어올 때마다 어색해지는 걸 한 번 겪고 나서야, 같은 도메인 안에서도 데이터 성격에 따라 저장소를 갈라두는 편이 결국 더 깔끔하다는 결론에 갔다.",
          ]}
        />
      </Section>
