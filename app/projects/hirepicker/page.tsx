  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "Refresh를 DB에 두고 매 요청마다 디스크를 읽는 구조를 운영해 보고 나서야, 토큰처럼 시간 지나면 사라지는 데이터는 트랜잭션 영역이 아니라는 감각이 잡혔다.",
            "단일 인스턴스에 Redis Pub/Sub을 깔아본 한 번이, ‘유명한 기술이라서’가 아니라 ‘내 운영 조건에 맞아서’를 기준으로 인프라를 정하게 만든 가장 비싼 수업이었다.",
            "Selenium으로 수집은 됐는데 광고가 그대로 노출되는 화면을 보고 나서야, 파이프라인의 품질이 수집보다 그 다음 단계에서 결정된다는 걸 받아들이게 됐다.",
          ]}
        />
      </Section>
