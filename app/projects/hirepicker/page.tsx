...
const troubles: Trouble[] = [
  ...
  },
  {
    ...
    lesson:
      "WebSocket에 Redis Pub/Sub을 깔던 시점에는 ‘다중 인스턴스 대비’라는 명분이 그럴듯해 보였는데, 실제로 띄워보니 인스턴스는 하나뿐이고 메시지 한 번에 외부 네트워크 I/O만 추가되는 구조였습니다. 그 회고가 Booming에서 SimpleBroker로 돌아갈 때 ‘환경을 먼저 보고 인프라를 정하자’는 기준이 됐고, 같은 함정에 다시 빠지지 않게 된 게 가장 큰 수확이었습니다.",
  },
  {
    ...
    lesson:
      "Selenium으로 수집까지 성공했을 때는 ‘됐다’ 싶었는데, 실제 서비스 화면에 광고 이미지가 그대로 박혀 있는 걸 보고 나서야 ‘수집 = 완료’가 아니라는 게 와닿았습니다. 그때 처음으로 수집·정제·검증을 분리해서 봐야 한다는 감각이 생겼고, 이후 다른 데이터 파이프라인을 짤 때도 정제 단계 자리를 먼저 비워두고 시작하게 됐습니다.",
  },
];
  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "처음엔 Refresh Token을 'DB에 회수 가능하게 두자' 싶어 RefreshToken 테이블에 박았는데, 매 인증 요청마다 디스크를 읽는 구조가 어색했습니다. 토큰처럼 시간이 지나면 사라지는 데이터는 트랜잭션 영역이 아니라 TTL 메모리에 둬야 한다는 감각이 그때 잡혔고, Booming에서 둘 다 Redis로 통합하면서 분명해졌습니다.",
            ...
            "Selenium으로 수집까지는 됐다 싶었는데, 실제 서비스 화면엔 광고 이미지가 그대로 박혀 있었습니다. 데이터 파이프라인의 품질은 수집보다 그 다음 단계(정제·검증)에서 결정된다는 걸 거기서 처음 봤고, 이후로는 정제 단계 자리를 먼저 비워두고 시작합니다.",
          ]}
        />
      </Section>
