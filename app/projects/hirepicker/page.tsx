  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "토큰처럼 시간이 지나면 사라지는 데이터는 트랜잭션 영역이 아니라 TTL 메모리에 둡니다. Refresh를 DB에 두고 매 요청마다 디스크를 읽는 구조를 운영해 보고 잡힌 감각.",
            "인프라는 '유명한 기술이라서'가 아니라 '내 운영 조건에 맞아서'로 고릅니다. 단일 인스턴스에 Redis Pub/Sub을 깔아본 한 번이 가장 비싼 수업이었습니다.",
            "데이터 파이프라인의 품질은 수집보다 그 다음 단계(정제·검증)에서 결정됩니다. Selenium으로 수집까지는 됐는데 광고가 그대로 노출되던 화면이 그 다음 단계 자리를 비워두는 습관의 시작이었습니다.",
          ]}
        />
      </Section>
