...
const troubles: Trouble[] = [
  {
    ...
    lesson:
      "처음엔 ‘JWT가 더 좋다던데’ 정도만 알고 도입을 고민했는데, 단일 서버 가정에 가장 잘 맞는 HttpSession으로 끝까지 가보고 나서야 어디가 부족한지가 또렷해졌습니다. 다음 프로젝트(HirePicker)에서 JWT로 옮겨갈 때 ‘왜 옮기는지’가 명확해진 게, 처음부터 JWT로 시작했을 때보다 훨씬 깊게 박혔습니다.",
  },
  {
    ...
    lesson:
      "버튼 위치 하나 옮기는 정도의 UI 수정에도 Action 자바 코드를 고치고 서버 재빌드를 기다려야 했고, 그 사이클을 며칠 반복하고 나서야 ‘왜 모두 프론트/백을 분리하는지’가 모범 사례 문장이 아니라 직접 겪은 비효율로 다가왔습니다.",
  },
];
  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "가장 단순한 형태로 끝까지 가보면 다음 프로젝트의 결정 근거가 또렷해집니다. 단일 서버·HttpSession·MyBatis로 마무리하고 나니 '왜 분리하고 왜 무상태로 가는지'가 그 다음 라운드(HirePicker)에서 분명히 보였습니다.",
            "'프론트/백 분리, 무상태 토큰, 연결 테이블 정합성' 같은 표현은 직접 빌드 사이클·세션 끊김·다대다 불일치를 며칠 겪고 나서야 다르게 읽힙니다. 책 한 권보다 한 번의 마찰이 빠릅니다.",
          ]}
        />
      </Section>
