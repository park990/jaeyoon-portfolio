  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "외부 API는 '받아주는 양'에 맞춰 동시 한도부터 잡고 붙입니다. KOSIS·HCX를 동기 루프로 두드리다 429를 맞고 배운 순서입니다.",
            "임베딩 점수가 높다고 맞는 통계는 아닙니다. 그래서 임베딩으로 후보만 좁히고, 단위·연도·범위는 결정론적 규칙으로 따로 검사합니다.",
            "도메인마다 컬럼이 달라지는 데이터라, 도메인별 테이블로는 새 도메인마다 마이그레이션이 걸렸습니다. EAV로 풀어두니 테이블 재정의 없이 새 도메인을 받을 수 있었고, 범위가 계속 바뀌는 PoC에선 그게 제일 컸습니다.",
            "수치 판정은 결정론적 코드, 자연어 설명은 LLM. 숫자 비교를 LLM에 맡기면 hallucination이 껴서, verifier와 explainer 역할을 아예 갈라놨습니다.",
            "재시도 로직은 llm_client.py 한 곳에만 둡니다. 호출부마다 흩어두면 한 군데를 빠뜨리기 쉽고, 일시적 429는 어차피 늘 생기는 일이기 때문입니다.",
          ]}
        />
      </Section>
