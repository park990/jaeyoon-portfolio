  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "외부 API는 'API가 받아주는 양'에 맞춰 동시 한도부터 정하고 붙입니다. 배치·세마포어·재시도 자리는 처음부터 비워두고요. KOSIS·HCX를 동기 루프로 두드리다 429를 맞은 게 이 습관의 시작이었습니다.",
            "임베딩 점수가 높다고 맞는 통계는 아닙니다. 코사인 0.85인데 단위·연도가 어긋난 표가 몇 번 잡힌 뒤로, 임베딩은 후보를 좁히는 데만 쓰고 단위·연도·범위 같은 결정론적 규칙으로 한 번 더 거릅니다.",
            "검증 데이터는 도메인마다 컬럼이 달라서 도메인별 테이블 대신 EAV로 풀었습니다. 새 도메인이 들어와도 테이블을 새로 정의할 필요가 없어, 범위가 계속 바뀌는 PoC 단계에선 이 확장성이 컸습니다.",
            "수치 비교를 LLM에 맡기면 hallucination이 낍니다. 그래서 단위·연도·오차율 판정은 verifier가 결정론적 코드로만 하고, 자연어 설명은 explainer에서 LLM이 따로 맡도록 역할을 나눴습니다. 이 경계가 판정 정확도엔 훨씬 안정적이었습니다.",
            "재시도는 호출부마다 흩지 않고 llm_client.py 한 곳에 모았습니다. 일시적 429는 어차피 늘 생기니 한곳에서 흡수하는 게 깔끔합니다.",
          ]}
        />
      </Section>
