...
const troubles: Trouble[] = [
  ...
  },
  {
    ...
    lesson:
      "코사인 0.85 이상이 나온 KOSIS 통계인데 막상 도메인적으로 보면 전혀 다른 지표인 경우를 몇 번 마주치고 나서야, 임베딩 점수만으로 ‘맞는 통계’를 고른다는 가정이 잘못이라는 걸 받아들였습니다. 그 뒤로는 임베딩으로 후보를 좁힌 다음 단위·연도·범위 같은 결정론적 조건으로 한 번 더 거르는 패턴을 기본 구조로 두게 됐습니다.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "동기 루프로 KOSIS·HCX를 두드리다 429를 받은 적이 있습니다. 그 한 번 이후로 새 외부 API를 붙일 때 가장 먼저 배치·세마포어·재시도 자리부터 비워두고 시작하고, 동시 한도는 '내가 보내고 싶은 양'이 아니라 '외부 API가 받아주는 양'에 맞춥니다.",
            "코사인 0.85가 나왔는데 단위·연도가 어긋난 통계를 몇 번 만나고 나서야 임베딩 점수만으로 '맞는 통계'를 고른다는 가정이 위험하다는 걸 알게 됐습니다. 그 뒤로는 임베딩으로 후보를 좁힌 다음 결정론적 규칙으로 한 번 더 거르는 패턴을 기본 구조로 둡니다.",
            "도메인마다 컬럼이 달라지는 검증 데이터를 도메인별 테이블이 아니라 EAV로 풀어둔 결정이, 나중에 새 도메인을 받는 자리를 그대로 살려뒀습니다. 사전 정의 없이 확장 가능한 게 PoC 단계에선 큰 자산이었습니다.",
            "수치 비교에 LLM을 쓰면 hallucination이 끼는 걸 본 뒤로 verifier는 단위·연도·오차율을 결정론적 코드로만 풀고, 자연어 설명은 explainer에서 LLM이 따로 하도록 분담을 갈랐습니다. 그 경계가 판정 정확도엔 훨씬 안정적이었습니다.",
            "호출부마다 try/except가 흩어져 있던 걸 한 곳으로 모아 정리하니 손대기도 쉽고 빠뜨리는 일도 없어졌습니다. 외부 LLM API의 일시적 429는 '항상 있을 일'로 두고 시작하는 게 맞다고 봅니다.",
          ]}
        />
      </Section>
