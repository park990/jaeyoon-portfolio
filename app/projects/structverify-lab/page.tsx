...
const KEY_DECISIONS = [
  {
    ...
    body:
      "도메인마다 필요한 컬럼이 달라 테이블을 미리 정의할 수 없었습니다. 인구통계는 '사망률·평균수명'이 컬럼이지만 농업은 '재배면적·수확량'이라, 도메인별 테이블로 가면 새 도메인이 들어올 때마다 컬럼 정의와 마이그레이션이 필요했습니다. 그래서 단일 테이블에 (entity, attribute, value) 형태로 속성을 행으로 푸는 EAV(Entity-Attribute-Value) 모델을 쓰고, parent_path 컬럼에 속성 간 계층(예: 인구 > 사망 > 영아사망률)을 함께 인코딩해 도메인별 마이그레이션 비용을 제거했습니다.",
  },
  {
  ...
  return (
    ...
    >
      ...
      <Section id="lessons" title="Lessons Learned">
        <LessonsList
          items={[
            "동기 루프로 KOSIS·HCX를 두드리다 몇 분 만에 429와 동시 호출 한도 경고를 동시에 받은 적이 있습니다. 그 한 번 이후로 새 외부 API를 붙일 때 가장 먼저 배치·세마포어·재시도 자리부터 비워두고 시작합니다.",
            "코사인 0.85가 나왔는데 단위·연도가 어긋난 통계를 몇 번 만나고 나서야 임베딩 점수만으로 '맞는 통계'를 고른다는 가정이 위험하다는 걸 인정했습니다. 그 뒤로는 임베딩으로 후보를 좁힌 다음 결정론적 규칙으로 한 번 더 거르는 패턴을 기본 구조로 둡니다.",
            "동시 호출 수를 키웠다가 오히려 429로 전체가 더 느려진 적이 있어요. 병렬화 한도는 '내가 보내고 싶은 양'이 아니라 '외부 API가 받아주는 양'에 맞춰야 결국 가장 빠릅니다.",
            "호출부마다 try/except가 흩어져 있던 코드를 utils/llm_client.py 한 곳으로 모아 정리하니 손대기도 쉽고 빠뜨리는 일도 없어졌습니다. 외부 LLM API의 일시적 429는 '항상 있을 일'로 두고 시작하는 게 맞다고 봅니다.",
            "팀원마다 Redis 포트·Java 버전이 달라 한참 같이 디버깅하던 시기를 보내고 나니, onboarding은 친절한 README가 아니라 `docker compose up` 한 줄로 풀어야 하는 문제라는 걸 받아들이게 됐습니다.",
          ]}
        />
      </Section>
